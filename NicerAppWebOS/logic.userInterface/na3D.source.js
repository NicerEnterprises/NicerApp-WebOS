...
import { OrbitControls } from '/NicerAppWebOS/3rd-party/3D/libs/three.js/examples/jsm/controls/OrbitControls-noPreventDefault.js';
            ...
            if (typeof itd == 'object' && itd!==null) {
                ...
                for (var i=0; i<6; i++) {
                    ...
                    if (itd[''+i] && itd[''+i].match(/.*\.png|.*\.jpeg|.*\.jpg|.*\.gif$/)) {
                        var
                        fullPath = it.filepath +'/'+ itd[''+i],
                        filename = fullPath.replace(/^.*[\\\/]/, ''),
                        path = fullPath.replace('/'+filename,''),
                        pathThumb = path+'/thumbs/'+filename;
debugger;
    ...
    onresize_do(t, callback) {
        ...
        for (var i=0; i<t.items.length; i++) {
            ...
            if (p) {
                ...
                itco = 75 * itcPercentage * it.maxColumnIta.maxColumn,
                itro = 75 * itrPercentage * it.maxRowIta.maxRow;
            ...
            if (it.model && p && p.model) {
                ...
                it.model.position.x = Math.round(
                    p.model.position.x 
                    + pitc 
                     + ((p.column-1)*100) + ((p.column-1)*pOffsetX_now)
                    + ( ( p.leftRight * (it.column-1) * 75))
                );
                it.model.position.y = Math.round(
                    p.model.position.y 
                    + pitr 
                    + ((p.row-1)*100) + ((p.row-1)*pOffsetY_now)
                    + ( (p.upDown *  (it.row-1) * 75))
                );
                it.model.position.z = -1 * ((it.level+1) * 350 );
                //debugger;
...
export class na3D_demo_cube {
    constructor(el,parent) {
        ...
        var materials = [
            ...
            new THREE.MeshBasicMaterial({
                map: new THREE.TextureLoader().load('/NicerAppWebOS/siteMedia/backgrounds/tiled/active/blue/blue170.jpg')
            }),
            new THREE.MeshBasicMaterial({
                map: new THREE.TextureLoader().load('/NicerAppWebOS/siteMedia/backgrounds/tiled/active/blue/abstract_water_texture-seamless.jpg')
            }),
            new THREE.MeshBasicMaterial({
                map: new THREE.TextureLoader().load('/NicerAppWebOS/siteMedia/backgrounds/tiled/active/orange/467781133_4f4354223e.jpg')
            }),
            new THREE.MeshBasicMaterial({
