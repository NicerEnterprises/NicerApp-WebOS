na.backgrounds = {
    ...
    next : function (div, search, url, saveTheme, callback, callStack) {
        ...
        //max = bgs.length,
        //min = 0,
        //collectionIdx = Math.floor(Math.random() * (max - min + 1) + min) - 1,
        hits = [];
        

        //while (collectionIdx < 0 || bgs[collectionIdx].files.length === 0) collectionIdx = Math.floor(Math.random() * (max - min + 1) + min) - 1;
